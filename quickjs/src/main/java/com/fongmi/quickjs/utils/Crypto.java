...
public class Crypto {
    ...
    public static String cdnDefend(String code) {
        try {
            int index = Character.digit(code.charAt(0), 16);
            if (index < 0) return "";
            MessageDigest digest = MessageDigest.getInstance("SHA-1");
            for (int i = 0; i < 1000000; i++) {
                byte[] bytes = digest.digest((code + i).getBytes(StandardCharsets.UTF_8));
                if (index + 1 < bytes.length && (bytes[index] & 0xFF) == 0xB0 && (bytes[index + 1] & 0xFF) == 0x0B) {
                    return "cdndefend_js_cookie=" + code + i;
                }
                digest.reset();
            }
        } catch (Exception e) {
