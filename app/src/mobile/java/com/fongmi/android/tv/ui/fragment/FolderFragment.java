...
public class FolderFragment extends BaseFragment {
    ...
    private VodFragment getParent() {
        return getParentFragment() instanceof VodFragment ? (VodFragment) getParentFragment() : null;
    }
    ...
    private HashMap<String, String> getExtend() {
        if (mType.getFilters() == null) return new HashMap<>();
    ...
    public void setResult(Result result) {
        if (!isAdded()) return;
        Optional.ofNullable(getChild()).ifPresent(f -> f.setResult(result));
    }
