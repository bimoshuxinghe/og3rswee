    ...
    public static int getNetworkState() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (nc == null) return 0;
                if (nc.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) return 1;
                if (nc.hasTransport(NetworkCapabilities.TRANSPORT_ETHERNET)) return 2;
            } else {
                NetworkInfo info = cm.getActiveNetworkInfo();
                if (info == null || !info.isConnected()) return 0;
                int type = info.getType();
                if (type == ConnectivityManager.TYPE_WIFI) return 1;
                if (type == ConnectivityManager.TYPE_ETHERNET) return 2;
            }
            return 0;
        } catch (Exception e) {
            return 0;
