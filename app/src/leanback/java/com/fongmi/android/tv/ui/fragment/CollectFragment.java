...
public class CollectFragment extends BaseFragment implements CustomScroller.Callback, VodPresenter.OnClickListener {
    ...
    private void setRecyclerView() {
        ...
        if (com.fongmi.android.tv.setting.Setting.getSearchMode() == 0) {
            mBinding.recycler.setHeader(getActivity(), R.id.result, R.id.recycler);
        } else {
            mBinding.recycler.setHeader(getActivity(), R.id.result);
            mBinding.recycler.setPadding(ResUtil.dp2px(8), mBinding.recycler.getPaddingTop(), mBinding.recycler.getPaddingEnd(), mBinding.recycler.getPaddingBottom());
        }
